"""Apply the additive area map layer migration using the configured PostgreSQL connection.

Set SUPABASE_DB_HOST and SUPABASE_DB_USER for a pooler connection when required.
No credentials are printed. Adds layer storage and an area-deletion cleanup trigger.
"""
import os
...
def main():
    ...
    try:
        with psycopg.connect(host=host, user=user, password=password, dbname="postgres",
            ...
            connection.execute((root / "migrations" / "20260917_area_map_layers.sql").read_text())
        print("Area map layer storage and deletion cascade are ready.")
